...
Func _MatchPicture($Match_Pic, $hWnd, $x1, $y1, $x2, $y2, $Threshold = 0.9, $LoopCount = 1, $LoopWait = 1)
	...
	For $iTries = 1 To $LoopCount Step +1
		...
		Local $hBitmap = _WinAPI_CreateCompatibleBitmap($hDC, $x2 - $x1, $y2 - $y1)
		Local $hOld = _WinAPI_SelectObject($hMemDC, $hBitmap)
		_WinAPI_BitBlt($hMemDC, 0, 0, $x2 - $x1, $y2 - $y1, $hDC, $x1, $y1, $SRCCOPY)
		_WinAPI_SelectObject($hMemDC, $hOld)
		_WinAPI_DeleteDC($hMemDC)
		_WinAPI_ReleaseDC($hWnd, $hDC)
		Local $hImage = _GDIPlus_BitmapCreateFromHBITMAP($hBitmap)
		Local $hMain_Pic = _Opencv_BMP2IPL($hImage)
		...
		If Not (DllStructGetData($tmaxloc, "x") = 0 And DllStructGetData($tmaxloc, "y") = 0 And $width2 = DllStructGetData($tmaxloc, "x") + $width2 And $height2 = DllStructGetData($tmaxloc, "y") + $height2) Then
			...
			Local $ret = [Int((($Coordinates[2]-$Coordinates[0])/2)+($Coordinates[0]+$x1)), Int((($Coordinates[3]-$Coordinates[1])/2)+($Coordinates[1]+$y1))]
			Return $ret
		EndIf
		Sleep($LoopWait)
	Next
	_cvReleaseImage($hMatch_Pic)
	_Internal_ErrorLogger("No match found. Loop counter: " & $LoopCount & ". // Total check time: " & Round(TimerDiff($Perf), 0) & " ms // Wait-Time per loop: " & $LoopWait & " ms. // Threshold: " & $Threshold & " // Match Image: ")
	Return SetError(2)
